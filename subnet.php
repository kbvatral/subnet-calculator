...
if(isset($_GET["ip1"])){
  ...
  $subnet_broadcast = $subnet_return[3];

  $num_subnets = pow(2, $network_bits);
  $num_host = pow(2, $required_bits);
  $num_usable = $num_host - 2;
...
function get_subnet($subnet_of_interest, $ip, $required_bits){
  ...
  return array($subnet_network, $subnet_start, $subnet_end, $subnet_broadcast);

}
function print_ip($ip_address){
  echo $ip_address[0],".",$ip_address[1],".",$ip_address[2],".",$ip_address[3];
}
